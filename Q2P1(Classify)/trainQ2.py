...
def training(a):
	...
	a.fit(x_train, y_train,epochs=20,batch_size=32,validation_data=(x_test[:int(0.5*len(x_test))],y_test[:int(0.5*len(y_test))]))
	a.evaluate(x_test[int(0.5*len(x_test)):],y_test[int(0.5*len(y_test)):],batch_size=32)
	a.save_weights(os.getcwd()+'/4c3dmcweights.h5')
	return a
